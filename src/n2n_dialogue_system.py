...
from src.intent_network import SentenceEncoder
from src.belief_tracker import InformSlotTracker, RequestSlotTracker
from src.belief_tracker import StateTracker
...
class Model(nn.Module):
    ...
    def __init__(self, onto, embed, conf, slot_trackers, kb):
        ...
        self.policy = PolicyNetwork(input_dim=self.state_tracker_hidden_size + conf["kb_indicator_len"],
        hidden_dim=self.state_tracker_hidden_size,
        output_dim=self.sent_group_size)
    ...
    def forward(self, usr_utts, kb_found, state_trk_hidden):
        ...
        sent_group_pred = self.policy(statereps_kb_found)
